...
int main(int argc, char **argv)
{
	...
	for(i=0; i<points_vector.size(); i++){
		cout << points_vector[i].x << " " << points_vector[i].y << "\n";
        
        if(min_x > points_vector[i].x){
            min_x = points_vector[i].x;
        }
        if(min_y > points_vector[i].y){
            min_y = points_vector[i].y;
        }
        if(max_x < points_vector[i].x){
            max_x = points_vector[i].x;
        }
        if(max_y < points_vector[i].y){
            max_y = points_vector[i].y;
        }
	}
  ...
  float x_scalefactor = float(win_width)/float(max_x - min_x);
  float y_scalefactor = float(win_height)/float(max_y - min_y);
  printf("x_scalefactor %f, y_scalefactor %f", float(x_scalefactor), float(y_scalefactor));
  ...
  if( XAllocNamedColor( display_ptr, color_map, "light grey", 
	...
	// 	cout << points_vector[i].x << " " << points_vector[i].y << "\n";
    //     XFillArc( display_ptr, win, gc_red, 
    //                    points_vector[i].x/win_width, points_vector[i].y/win_height,
    //                    win_height/50, win_height/50, 0, 360*64);
	//   }
  ...
  while(1)
    ...
    //         cout << x_scalefactor * (points_vector[i].x - min_x) << " final x\n";
    //         cout << y_scalefactor * (points_vector[i].y - min_y) << "final y \n";
    //     }

      for(int i=0; i<points_vector.size();i++){
        printf("New x %f, New y %f -------", x_scalefactor, y_scalefactor);
